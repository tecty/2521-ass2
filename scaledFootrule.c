...
*  First we generate a matrix-like list to record any possible distance for each
*  page in different positions. Then, we use a priority queue to sort the list
*  and leave the smallest distance node in the queue, which contain its
*  page_name and derived proper rank. After the queue is empty, generate a
*  link-list of urls that contains the final order of page_names. Finally, we
*  print all the pagenames in the link list and the minimal total distance.
*
